    ...
    methods:{
      ...
      getSectionInfo(id, name){
        ...
        if(isAndroid&& isInsert ){
          let  datas={url:"https://www.anzhique.com/dataAdmin/#/m-project?id="+id+"&type=5"}
          let data=JSON.stringify(datas)
            window.AzqBridge.openWebview(data)
        }else if(isiOS && this.$route.query.iOS==1){
             window.webkit.messageHandlers.openWebview.postMessage("https://www.anzhique.com/dataAdmin/#/m-project?id="+id+"&type=5&iOS=1")
        }else{
            this.$router.push({
              path:'/m-project',
              query:{
