<template>
  <div class="mainCounts">
		<div class="header">
      <div class="titleName">集团数据看板</div>
    ...
    methods:{
      ...
      getSectionInfo(id, name){
        ...
        if(isAndroid&& isInsert ){
          let  datas={url:"https://www.anzhique.com/dataAdmin/#/m-command?id="+id+"&type=2"}
          let data=JSON.stringify(datas)
            window.AzqBridge.openWebview(data)
        }else if(isiOS &&  this.$route.query.iOS==1){
             window.webkit.messageHandlers.openWebview.postMessage("https://www.anzhique.com/dataAdmin/#/m-command?id="+id+"&type=2&iOS=1")
        }else{
            this.$router.push({
            path:'/m-command',
            query:{
